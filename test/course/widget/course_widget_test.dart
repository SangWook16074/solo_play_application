...
void main() {
  ...
  group("Course Widget Tests", () {
    setUpAll(() {
      widget = const MaterialApp(home: CourseWidget());
    });
    testWidgets("Course Widget이 올바르게 화면에 표시된다.", (tester) async {
      await tester.pumpWidget(widget);
      expect(find.byKey(const Key("Course Widget Map")), findsOneWidget);
      expect(
          find.byKey(const Key("Course Widget Description")), findsOneWidget);
    });
